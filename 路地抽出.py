...
#道路中心線の読み込み
road_center = gpd.read_file('根津二丁目\RdCL_根津二丁目.shp')
road_center_proj = road_center.to_crs('EPSG:6677') #CRSの変更
print(road_center_proj)

#道路縁の読み込み
road_edge = gpd.read_file('根津二丁目\RdEdg_根津二丁目.shp')
road_edge_proj = road_edge.to_crs('EPSG:6677') 

def get_segments(line_string):
    starts = line_string.coords
...
# 端からある長さ at_length の座標を求める; 線分を1mごとに区切って各点の座標を求めるため
...
def evaluate_by_interval(line_string, interval=1.0):
  ...
  cumulative_lengths = np.cumsum(lengths) #長さの累積和
...
road_edge_union = road_edge_proj['geometry'].union_all()
ray_length = 20
...
road_widths = road_widths_at_points[['rID', 'raytraced_width']].groupby(['rID']).median() #各点の幅員平均をとることで、rIDごとの道路幅員を計算
...
road_widths.to_csv('根津二丁目road_width.csv')
"""
import csv
with open('road.width.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(road_widths)

with open('road.width.csv') as f:
    print(f.read())
"""
